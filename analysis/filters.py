...
class Filter():
    ...
    def parse_fclk_actual(self):
        """ Parse the achieved clock frequency (in MHz) from the output files """
        met_timing= _run_cmd(f'grep -c "All user specified timing constraints are met" {self.synth_dir}/post_route_timing.rpt')
        passed = met_timing[0] == '1'
        
        wns_timing= _run_cmd(f'grep -B 3 " met." {self.synth_dir}/post_route_timing.rpt | head -n 1 | tr -s \' \' | cut -d\' \' -f2')
        clkT= _run_cmd(f'grep -A 2 "Clock  Waveform(ns)" {self.synth_dir}/post_route_timing.rpt | tail -n 1 | tr -s \' \' | cut -d\' \' -f4')
    ...
    def parse_results(self):
        ...
        lut_result= _run_cmd(f'grep "CLB LUTs" {self.synth_dir}/post_route_util.rpt | cut -d"|" -f3')
        reg_result= _run_cmd(f'grep "CLB Registers" {self.synth_dir}/post_route_util.rpt | head -n 1 | cut -d"|" -f3')
        clb_result= _run_cmd(f'grep "| CLB  " {self.synth_dir}/post_route_util.rpt | head -n 1 | cut -d"|" -f3')
        dsp_result= _run_cmd(f'grep "| DSPs  " {self.synth_dir}/post_route_util.rpt | head -n 1 | cut -d"|" -f3')
        ...
        try:
            return {
                ...
                'parallelism': self.p,
                'fclk_actual': self.parse_fclk_actual(),
...
class ClashFilter(Filter):
    ...
    def impl(self):
        ...
        with open(f'{self.synth_dir}/Filter.hs', 'w') as f:
                f.write(self.src)
        with open(f'{self.synth_dir}/synth.xdc', 'w') as f:
            f.write(f"""
...
""")
        ...
        _run_cmd(f'nix-shell --command \'make -C {self.synth_dir}\' ../shell.nix &> {self.synth_dir}/build.log')
...
class SsrFilter(Filter):
    ...
    def impl(self):
        ...
        with fileinput.FileInput(f'{self.synth_dir}/synth.tcl', inplace=True) as f:
            for line in f:
